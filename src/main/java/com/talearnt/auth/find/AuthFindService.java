...
import com.talearnt.user.infomation.repository.FindPasswordUrlRepository;
...
import com.talearnt.auth.verification.VerificationMapper;
import com.talearnt.auth.verification.repository.PhoneVerificationTraceRepository;
...
public class AuthFindService {
    ...
    private final AuthFindQueryRepository authFindQueryRepository;
    ...
    public ResponseEntity<CommonResponse<String>> sendAuthenticationCode(String phoneNumber, String name){
        log.info("유저 아이디 찾기 문자 전송 시작 : {}, {}",phoneNumber,name);
        //Paramter 값이 제대로 넘어 왔는 지 검증
        if (name == null){
            log.error("유저 아이디 찾기 문자 전송 실패 - name 값이 null : {}",ErrorCode.BAD_PARAMETER);
        ...
        log.info("유저 아이디 찾기 문자 전송 끝 : {}",response);
     ...
     * @return boolean
     * */
    public AuthFindResDTO checkVerificationCode(CheckUserVerificationCodeReqDTO reqDTO){
        ...
        PhoneVerification phoneVerification = verificationCodeQueryRepository.selectPhoneVerificationByMinutesAndPhone(reqDTO.getPhone())
                .orElseThrow(()->{
                    log.error("유저 아이디 찾기 인증 번호 검증 실패 - 해당 휴대폰 번호로 발송된 번호 없음 : {}",ErrorCode.AUTH_NOT_FOUND_PHONE_CODE);
     ...
     * */
    public AuthFindResDTO findUserIds(CheckUserVerificationCodeReqDTO checkUserVerificationCodeReqDTO){
        ...
        AuthFindResDTO authFindResDTO = authFindQueryRepository.findUserIdAndCreatedAt(checkUserVerificationCodeReqDTO.getPhone())
                .orElseThrow(()->{
        ...
        message.setText("Talearnt 회원님의 아이디는 [ " + authFindResDTO.getUserId() + " ] 입니다.");
    ...
    public ResponseEntity<CommonResponse<AuthFindResDTO>> sendEmailForPwd(String userId, String phone) throws MessagingException {
        ...
        //데이터 베이스에 저장
        FindPasswrodUrl pwdUrl = AuthFindMapper.INSTANCE.toFindPasswordUrlEntity(checkAuth.getUserId(), uuid);
    ...
    public ResponseEntity<CommonResponse<String>> changeUserPassword(Long findPwdUrlNo, String uuid, CheckUserPwdReqDTO checkUserPwdReqDTO){
        ...
        String userId = authFindQueryRepository.findUserIdByUrlNoAndUuid(findPwdUrlNo,uuid)
                .orElseThrow(()->{
        ...
        User user = authFindQueryRepository.findUserByUserId(userId)
                .orElseThrow(()->{
